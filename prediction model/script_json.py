...
# Passengers a single bus can serve across the time window
# (≈ seated capacity × trips achievable in the window). Demand a route
# captures is capped at this value, so "served" reflects real vehicle limits.
BUS_CAPACITY     = 320.0
...
# ── ROUTE OPTIMISER (greedy construction + 2-opt local search) ────────────────

def path_time(order: list) -> float:
    """Total shortest-path travel time along an ordered open route."""
    return sum(
        ALL_PATH_LENGTHS[order[i]].get(order[i + 1], float("inf"))
        for i in range(len(order) - 1)
    )


def two_opt(order: list) -> list:
    """2-opt local search: repeatedly reverse a sub-segment if it shortens the
    open route. The start stop is held fixed (the bus begins where demand is
    highest). Returns a reordering of the same stop set with travel time <= the
    input. Runs to a local optimum (no improving 2-swap remains)."""
    if len(order) < 4:
        return order[:]
    best = order[:]
    best_t = path_time(best)
    improved = True
    while improved:
        improved = False
        # i >= 1 keeps the first stop fixed; j spans to the end (open path).
...
def greedy_route(demand: dict) -> list:
    ...
    for bus_idx in range(N_BUSES):
        ...
        while True:
            # Stop extending once the bus is at passenger capacity.
        ...
        # Refine visiting order with 2-opt (reduces deadhead travel time).
        ...
        routes.append({
            ...
            # Demand actually carried is capped by vehicle capacity.
