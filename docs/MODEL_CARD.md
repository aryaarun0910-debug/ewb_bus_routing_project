...
| Metric | Value | Split |
|---|---|---|
| **R² (primary, reported)** | **0.9421** (RMSE 4.13, MAE 2.11 boardings) | Temporal — train 2023, test unseen 2024 |
| R² (random split) | 0.9424 (RMSE 4.15) | Random 80/20 |
...
### What R² = 0.9421 does — and does not — mean
...
|---|---|---|
| Random vs. temporal split | R² 0.9424 vs. 0.9421 (gap = 0.0002) | Row-level autocorrelation inflating the headline score |
| Anchor sensitivity (±20% perturbation of the smartcard demand anchor) | R² spread = 0.0002 (0.9412 / 0.9413 / 0.9414 for baseline / −20% / +20%) | The result being an artefact of the *exact magnitude* of one decade-old, concessionary-only data source — the model is learning demand *shape*, not memorising one source's scale |
| Year shift (train 2023→test 2024 and reverse) | avg R² = 0.9405 | The model memorising one year's idiosyncrasies rather than stable structure |
| Season shift (train winter→test summer and reverse) | avg R² = 0.9257 | — and quantifies the honest bound: a ~0.016 drop is the expected, informative cost of extrapolating across a seasonal regime change |

**Interpretation**: the 0.9421 headline is stable under every stress test we
could construct from available data. The one consistent, expected weak point
is cross-season transfer (0.9257 vs. 0.9421 in-distribution) — exactly what
...
> curves or event/disruption logs exist for Ladywood stops. The 0.9421 R² should
...
- **Independence**: demand at the same stop an hour apart is highly
  ...
  **This is precisely why we report the temporal split (train 2023 → test
  ...
  0.0002 gap between them (0.9421 vs. 0.9424) is our direct, quantified evidence
...
order of magnitude to take seriously: our 0.9421 could plausibly become
something closer to Instacart's 61% under an equivalent shock, not just dip a
few points the way our season-shift check (0.9257) suggests for *ordinary*
seasonal variation.
